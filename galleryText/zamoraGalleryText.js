import CenterGrayTitle from "../components/centerGrayTitle.js";
import CenterBlackTitle from "../components/centerBlackTitle.js";
import GalleryLineation from "../components/galleryLineation.js";
import styles from "../styles/Zamora.module.css";

export function ZamoraRawText1() {
  return (
    <>
      <CenterGrayTitle>
        <span className={styles.blackShadow}>felicia zamora</span>
      </CenterGrayTitle>
    </>
  );
}

export function ZamoraRawText2() {
  return (
    <>
      <CenterBlackTitle>
        <span className={styles.grayShadow}>selections from 'quotient'</span>
      </CenterBlackTitle>
    </>
  );
}

export function ZamoraRawText3() {
  return (
    <>
      <GalleryLineation>{`
                        what is still is not
                        contained. & the sallow
                        sees itself as limb, as broad-leaf
                        extension
                        in earth as earth. A delving root
                        stretches inside, burgeons out —
                        anchors to pond’s surface
                        & heaves with rhythms of weather
                        & surface, here, loves restless.
                        Yearns to permeate, to divest.
        `}</GalleryLineation>
    </>
  );
}

export function ZamoraRawText4() {
  return (
    <>
      <GalleryLineation>
        {" "}
        {`
                        Interrobang, a gathering of sorts.
                        Synesthesia evokes before
                        speech lost in its own
                        trembling throat; language mutates
                        with us — `}{" "}
        <i>pick a number</i>
        {`
                        & the card game becomes
                        mathematical. Creation
                        in sequence before — the counters
                        of numbers only count. Quantity
                        allows observance. `}
        <i>How do you spell</i>{" "}
        {`— ? Observe
         `}
      </GalleryLineation>
    </>
  );
}

export function ZamoraRawText5() {
  return (
    <>
      <GalleryLineation>
        {" "}
        {`
                        & what forms magic
                        originates as magic. We think
                        in `}{" "}
        <i>inventory</i>{" "}
        {`to lock universals
                        in brainwaves & tongues. `}
        <i>Ownership</i>
        {`
                        out of word is dangerous. `}
        <i>Forget</i>
        {`,
                        the same. No matter
                        calls forth without action. Words
                        reveal magical elements.
                        A defect of magic:
                        thinking it is magician.
          `}
      </GalleryLineation>
    </>
  );
}

export function ZamoraRawText6() {
  return (
    <>
      <GalleryLineation>
        {" "}
        {`
                        Numerator thrones double
                        line over line hover, verge —
                        cross hatch
                        the mold, to make
                        figure obey the clay,
                        the design. What division
                        strips. Naked & exposed. The source
                        no longer boon; ear no longer bent
                        in heed. What reveals
                        in the shuck. What visions —
          `}
      </GalleryLineation>
    </>
  );
}

export function ZamoraRawText7() {
  return (
    <>
      <GalleryLineation>
        {" "}
        {`
                        How eyelashes & tree branches
                        cast similar shadows — a game
                        of proportions. & when we admit:
                        `}
        <i>My magnitude hides, swallows</i>
        {`
                        `}
        <i>in this body</i>
        {` — a grave
                        tucks in fissures of hemispheres,
                        mirroring. In the cerebral cortex,
                        we fall into lobes & bones. Sutures
                        of intersection. We count: `}
        <i>one one-thousand</i>,
        {`
                        `}
        <i>two one-thousand</i>
        {`, under folds, in wait of boom.
           `}
      </GalleryLineation>
    </>
  );
}

export function ZamoraRawText8() {
  return (
    <>
      <GalleryLineation>
        {" "}
        {`
                        Inverse — tongue
                        a pendulum. Nests of mud
                        sculpt in the lighthouse rafters
                        where we juggle eggs to and fro
                        until the swallows
                        peck our knuckles raw.
                        In the blood, in the shell-
                        scatter, a crime scene
                        to witness: the cold
                        potter’s wheel, spins still.
            `}
      </GalleryLineation>
    </>
  );
}
